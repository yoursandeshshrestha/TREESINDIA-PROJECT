...
export default function FixedPriceServicesSection() {
  const dispatch = useAppDispatch();
  const router = useRouter();
  const { location, isLoading: locationLoading } = useLocation();

  // Create filters for fixed price services
  ...
  // Use TanStack Query to fetch fixed price services
  const {
    data: response,
    isLoading,
    error,
  ...
  const handleViewAllServices = () => {
    router.push("/services?type=fixed-price");
  };

  const getSectionTitle = () => {
    if (location?.city && location?.state) {
      return `Fixed Price Services in ${location.city}`;
    }
    return "Fixed Price Services";
  };

...
export default function InquiryServicesSection() {
  const dispatch = useAppDispatch();
  const router = useRouter();
  const { location, isLoading: locationLoading } = useLocation();

  // Create filters for inquiry services
  ...
  // Use TanStack Query to fetch inquiry services
  const {
    data: response,
    isLoading,
    error,
  ...
  const handleViewAllServices = () => {
    router.push("/services?type=inquiry-based");
  };

  const getSectionTitle = () => {
    if (location?.city && location?.state) {
      return `Inquiry Services in ${location.city}`;
    }

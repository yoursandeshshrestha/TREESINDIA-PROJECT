...
export default function HomeServicesSection() {
  ...
  // Create filters for home services
  ...
  // Use TanStack Query to fetch home services
  const {
    data: response,
    isLoading,
    error,
  ...
  const handleViewAllServices = () => {
    router.push("/services?category=Home+Services");
  };

  const getSectionTitle = () => {
    if (location?.city && location?.state) {
      return `Home Services in ${location.city}`;
    }
